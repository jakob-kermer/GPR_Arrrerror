...
public class GameManager : MonoBehaviour
{
    ...
    public IEnumerator EnableTargetSelection_Player()
    {
        ...
        foreach (Player player in players)
        {
            // disables the player's selector
    ...
    {
        this.state = GameState.PlayerTurn; // reset game state to prevent target selection

        foreach (Enemy enemy in enemies)
        {
            // disables the enemy's selector to not show up when hovering over them
            enemy.EnableSelector = false;
        }
        foreach (Player player in players)
        {
            // disables the player's selector to not show up when hovering over them
            player.EnableSelector = false;
        }
